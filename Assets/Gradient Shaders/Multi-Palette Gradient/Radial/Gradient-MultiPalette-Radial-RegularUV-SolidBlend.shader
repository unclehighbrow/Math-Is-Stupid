Shader "Gradient/Multi Palette/Radial/Regular UV/Solid Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed4 tex=tex2D(_PaletteTex, half2(lerp(0,1.0,sqrt( (i.uv.x*i.uv.x)+(i.uv.y*i.uv.y) ))+_PaletteOffset,_PaletteRow) );	//Output paletted radial gradient
				return tex;
			}
