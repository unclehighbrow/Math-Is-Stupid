Shader "Gradient/No Texture/Box3/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			VertexToFragment vert(AppData v) {
    	        ...
    	        float cosX = cos(ang);
        	    float sinY = sin(ang);
            	float2x2 rotationMatrix = float2x2(cosX,-sinX,sinY,cosX);	//Position and rotation
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color3,lerp(_Color,_Color2,abs(i.uv.x)),abs(i.uv.y)));	//Output box3 gradient
			}
