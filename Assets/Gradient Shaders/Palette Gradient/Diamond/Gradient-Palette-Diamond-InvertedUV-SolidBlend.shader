Shader "Gradient/Palette/Diamond/Inverted UV/Solid Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed4 tex=tex2D(_PaletteTex, half2(lerp(1.0,0,abs(i.uv.x)+abs(i.uv.y))+_PaletteOffset ,0) );	//Output paletted diamond gradient
				return tex;
			}
