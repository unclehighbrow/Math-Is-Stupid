Shader "Gradient/Single Texture/Box6/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				left=lerp(_Color4,_Color5,uvx1+_EdgeOffset);	//Interpolate left bottom
				right=lerp(_Color5,_Color6,uvx2+_EdgeOffset);	//Interpolate right bottom
				fixed4 mid2=lerp(left,right,i.uv2.x+_EdgeOffset);	//Interpolate bottom
				fixed4 blended=lerp(mid2,mid,i.uv2.y);			//Find overall bottom
				fixed4 tex=tex2D(_MainTex,i.uv);	//Get texture
				return tex * blended;				//Output Box6 blended gradient
			}
