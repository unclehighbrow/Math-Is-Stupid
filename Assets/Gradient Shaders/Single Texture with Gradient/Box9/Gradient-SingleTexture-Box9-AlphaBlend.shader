Shader "Gradient/Single Texture/Box9/Alpha Blend" {
	...
	SubShader {
		...
		Tags {"Transparent" = "Geometry" "RenderType" = "Transparent"}		//Background | Geometry | AlphaTest | Transparent | Overlay
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				half uvx1=i.uv2.x*2;			//For speed, left gradient
				half uvx2=(i.uv2.x-0.5)*2;	//For speed, right gradient
				fixed4 left;
				fixed4 right;
				fixed4 mid;
				left=lerp(_Color1,_Color2,uvx1);	//Interpolate left top
				right=lerp(_Color2,_Color3,uvx2);	//Interpolate right top
				mid=lerp(left,right,i.uv2.x);		//Interpolate top
				fixed4 mid2;
				...
				mid2=lerp(left,right,i.uv2.x+_MiddleOffset);		//Interpolate mid
				fixed4 top=lerp(mid2,mid,(i.uv2.y-0.5)*2);		//Find overall top
				...
				fixed4 mid3=lerp(left,right,i.uv2.x+_EdgeOffset);	//Interpolate bottom
				fixed4 bot=lerp(mid3,mid2,i.uv2.y*2);			//Find overall bottom
				fixed4 blended=lerp(bot,top,i.uv2.y);			//Interpolate bottom and top
				fixed4 tex=tex2D(_MainTex,i.uv);				//Get texture
				return tex*blended;					//Output Box9 blended gradient
			}
