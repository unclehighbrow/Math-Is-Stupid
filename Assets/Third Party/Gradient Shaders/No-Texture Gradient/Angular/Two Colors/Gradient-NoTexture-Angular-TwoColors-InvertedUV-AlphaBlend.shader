Shader "Gradient/No Texture/Angular/Two Colors/Inverted UV/Alpha Blend" {
		...
		Pass {
			...
			VertexToFragment vert(AppData v) {
            	...
            	float2x2 rotationMatrix = float2x2(cosX,-sinX,sinY,cosX);
           		o.uv = mul(v.texcoord.xy, rotationMatrix);	//Position and rotate
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color2,_Color,(atan2(i.uv.y,i.uv.x)*0.1591549)+0.5 ));	//Output angular gradient (0.1591549 is 1.0/(2*PI))
			}
