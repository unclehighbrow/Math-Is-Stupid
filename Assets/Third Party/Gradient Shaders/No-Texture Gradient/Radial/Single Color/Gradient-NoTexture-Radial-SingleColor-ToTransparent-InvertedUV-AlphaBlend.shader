Shader "Gradient/No Texture/Radial/Single-Color/To Transparent/Inverted UV/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(fixed4(_Color.rgb,0),_Color,sqrt( (i.uv.x*i.uv.x)+(i.uv.y*i.uv.y) )+_Offset ));	//Output radial gradient
			}
