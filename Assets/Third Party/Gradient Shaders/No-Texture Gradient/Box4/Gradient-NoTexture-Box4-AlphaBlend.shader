Shader "Gradient/No Texture/Box4/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(   lerp(_Color3,_Color4,abs(i.uv.x)),   lerp(_Color,_Color2,abs(i.uv.x)+_EdgeOffset),    abs(i.uv.y)   ));	//Output box4 gradient
			}
