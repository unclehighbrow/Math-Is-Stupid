Shader "Gradient/Palette/Radial/Inverted UV/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed4 tex=tex2D(_PaletteTex, half2(lerp(1.0,0,sqrt( (i.uv.x*i.uv.x)+(i.uv.y*i.uv.y) ))+_PaletteOffset,0) );	//Output paletted radial gradient
				return tex;
			}
