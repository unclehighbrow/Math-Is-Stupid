Shader "Gradient/Palette/Box4/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(   tex2D(_PaletteTex,half2(lerp(0,1,abs(i.uv.x)),0)),   tex2D(_PaletteTex2,half2(lerp(0,1,abs(i.uv.x))+_PaletteOffset,0)),    abs(i.uv.y)   ));	//Output dual-paletted box4 gradient
			}
