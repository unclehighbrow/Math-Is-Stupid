Shader "Gradient/Palette/Box3/Solid Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color,tex2D(_PaletteTex,half2( lerp(0,1.0,abs(i.uv.x)),0)), abs(i.uv.y)));	//Output paletted box3 gradient
			}
