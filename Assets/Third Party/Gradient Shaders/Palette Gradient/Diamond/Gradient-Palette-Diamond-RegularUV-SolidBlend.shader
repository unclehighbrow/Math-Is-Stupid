Shader "Gradient/Palette/Diamond/Regular UV/Solid Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				fixed4 tex=tex2D(_PaletteTex, half2(lerp(0,1.0,abs(i.uv.x)+abs(i.uv.y))+_PaletteOffset ,0) );	//Output paletted diamond gradient
				return tex;
			}
