Shader "Gradient/Multi Palette/Box3/Alpha Blend" {
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(_Color,tex2D(_PaletteTex,half2( lerp(0,1.0,abs(i.uv.x)),_PaletteRow)), abs(i.uv.y)));	//Output paletted box3 gradient
			}
