Shader "Gradient/Multi Palette/Box4/Alpha Blend" {
	...
	Properties {
		...
		_PaletteTex ("Palette Texture 1", 2D) = "" {}				//Receive input from a Texture on Texture Unit 0
		_PaletteTex2 ("Palette Texture 2", 2D) = "" {}				//Receive input from a Texture on Texture Unit 0
		_PaletteOffset ("Palette Offset", float) = 0	//Receive input from a float
		_PaletteRow ("Palette Row 1", float) = 0			//Receive input from a float
		_PaletteRow2 ("Palette Row 2", float) = 0			//Receive input from a float
	}
	...
	SubShader {
		...
		Pass {
			...
			uniform sampler2D _PaletteTex2;					//Define _PaletteTex2 from Texture Unit 0 to be sampled in 2D
			//uniform float4 _PaletteTex2_ST;					//Use the Float _PaletteTex2_ST to pass the Offset and Tiling for the texture(s)
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				return fixed4(lerp(   tex2D(_PaletteTex,half2(lerp(0,1,abs(i.uv.x)),_PaletteRow)),   tex2D(_PaletteTex2,half2(lerp(0,1,abs(i.uv.x))+_PaletteOffset,_PaletteRow2)),    abs(i.uv.y)   ));	//Output dual-paletted box4 gradient
			}
