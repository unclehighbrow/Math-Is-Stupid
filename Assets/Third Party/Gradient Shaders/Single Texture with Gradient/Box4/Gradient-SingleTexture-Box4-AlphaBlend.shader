Shader "Gradient/Single Texture/Box4/Alpha Blend" {
	...
	Properties {
		...
		_Color3 ("Third Color", Color) = (1,1,1,1)		//Receive input from a fixed Color
		_Color4 ("Fourth Color", Color) = (1,1,1,1)		//Receive input from a fixed Color
	...
	SubShader {
		...
		Pass {
			...
			fixed4 frag(VertexToFragment i) : COLOR {
				...
				return tex * fixed4(lerp(   lerp(_Color3,_Color4,abs(i.uv2.x)),   lerp(_Color,_Color2,abs(i.uv2.x)+_EdgeOffset),    abs(i.uv2.y)   ));	//Output textured box4 gradient
			}
